...
private:
    ...
    sf::Text           crearTexto(const std::string& str, unsigned int tam,
                                   sf::Color color, float x, float y);
    sf::RectangleShape crearCajaTecla(float x, float y, float ancho, float alto);
    sf::Text           crearLabelTecla(const std::string& label,
                                        sf::RectangleShape& caja);
    sf::Sprite         crearSpritePU(const std::string& nombre,
                                      float x, float y, float altoDeseado);
};