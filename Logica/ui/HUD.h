...
public:
    ...
    void actualizar(const DatosJugador& j1, const DatosJugador& j2, float tiempoRestante, int turnoActual, EstadoJuego estado, int cantPowerUpsActivo);
    void dibujar();
    ...
    bool clickEnPowerUp(int mouseX, int mouseY) const;
private:
    ...
    void actualizarBotones(EstadoJuego estado, int cantPowerUpsActivo);
    void dibujarFila(const FilaTanque& fila);
    bool dentroDeRect(const sf::RectangleShape& rect, int mx, int my) const;
    sf::RectangleShape botonPowerUp;
    sf::Text textoPowerUp;
    sf::Text estadoPowerUp;
};